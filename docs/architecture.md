...
- **services/sports_intel/** — sports-specific checks (jersey consistency, logo consistency, stadium consistency, crowd anomaly detection, broadcast verification).
- **services/explainability/** — turns raw detector outputs into human-readable explanations and highlighted-frame evidence.
...
| `POST /api/v1/analysis/{media_id}` | Trigger analysis on uploaded media |
| `GET /api/v1/analysis/{analysis_id}/status` | Poll analysis status |
